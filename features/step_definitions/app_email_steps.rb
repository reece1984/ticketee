...
Given /^ActionMailer delivers via SMTP$/ do
...
@mails = @gmail.inbox.find(:unread, :from => "jamesreece84@googlemail.com") do |mail|
		if mail.message.subject =~ /^\[ticketee\]/
			# mail.delete!  I get a testing error when trying to delete the email
			@received_mail = true
		end
	end
